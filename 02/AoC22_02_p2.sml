...
fun score (data : string list, n : int, len : int, total : int) = if n<len then (
    ...
    in
        ...
        if first="A" then (
            if last=" Z" then tt := !tt+2 else if last=" Y" then tt := !tt+1 else tt := !tt+3
        ) else if first="B" then (
            if last=" Z" then tt := !tt+3 else if last=" Y" then tt := !tt+2 else tt := !tt+1
        ) else if first="C" then (
            if last=" Z" then tt := !tt+1 else if last=" Y" then tt := !tt+3 else tt := !tt+2
        ) else (
            tt := !tt
        );
